...
class MenuManagerController extends Controller
{
  ...
  public function index()
  {
    ...
    $config['form'] = (object)[
      'method' => 'POST',
      'action' => route('panel.menu-manager.store')
    ];
    $sortable = self::getMenu();
    return view('panel.menumanager.index', compact('config', 'sortable'));
  }
  ...
  public function store(Request $request)
  {
    ...
    if ($validator->passes()) {
      ...
      try {
        ...
        DB::commit();
        $response = response()->json($this->responseStore(true, NULL, route('panel.menu-manager.index')));
  ...
  public function edit($id, Request $request)
  {
    ...
    $config['form'] = (object)[
      'method' => 'PUT',
      'action' => route('panel.menu-manager.update', $id)
    ];
    $data = MenuManager::find($id);
    $sortable = self::getMenu();
    return view('panel.menumanager.index', compact('config', 'sortable', 'data'));
  }
  ...
  private static function tree($roots)
  {
    ...
    foreach ($roots as $item) {
      ...
      $html .= '<a href="' . route('panel.menu-manager.edit', $item->id) . '" class="btn btn-sm btn-default"><i class="fa fa-fw fa-edit"></i></a>
                        <button
                          type="button"
