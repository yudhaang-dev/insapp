...
use App\Models\MenuManagerRole;
...
//    $this->middleware('can:role-delete', ['only' => ['destroy']]);
  ...
  public function index(Request $request)
  {
    ...
    if ($request->ajax()) {
      ...
      return DataTables::of($data)
        ...
        ->addColumn('action', function ($row) {
          $btnDelete = $row->id != 1 ? '<li><a class="dropdown-item btn-delete" href="#" data-id ="'.$row->id.'" >Hapus</a></li>' : '';
          $actionBtn = '
          <div class="dropdown d-inline-block">
              ...
              <ul class="dropdown-menu dropdown-menu-end">
                 <li><a class="dropdown-item" href="'.route('panel.roles.edit', $row->id).'">Edit</a></li>
                '.$btnDelete.'
              </ul>
          </div>';
    ...
    return view('panel.roles.index', compact('config'));
  }
  ...
  public function create()
  {
    ...
    $config['breadcrumbs'] = [
      ['url' => route('panel.roles.index'), 'title' => "Role"],
      ['url' => '#', 'title' => "Tambah Role"],
    ];
    $config['form'] = (object) [
      'method' => 'POST',
      'action' => route('panel.roles.store')
    ];
    $permissions = self::getStaticMenu();
    return view('panel.roles.form', compact('config', 'permissions'));
  }
  ...
  public function store(Request $request)
  {
    ...
    if ($validator->passes()) {
      ...
      try {
        ...
        DB::commit();
        $response = response()->json($this->responseStore(true, null, route('panel.roles.index')));
  ...
  public function edit($id)
  {
    ...
    $config['form'] = (object) [
      'method' => 'PUT',
      'action' => route('panel.roles.update', $id)
    ];
    return view('panel.roles.form', compact('config', 'data', 'permissions'));
  }
          ...
          <td>
              <div class="form-check form-check-inline">
                  <input class="form-check-input menu-item" type="checkbox" name="menus[][menu_manager_id]" value="'.$item->id.'" id="'.$item->id.$item->title.'"
                   onclick="'.($item->type == 'module' ? "return false;" : null).'"'.(isset($role_menu) && in_array($item->id,
        $role_menu) ? 'checked' : '').'>
      ...
      foreach ($permissions ?? array() as $permission) {
        $html .= '
              ...
              <div class="form-check form-check-inline">
                  ...
                  id="'.$permission->id.'" '.(isset($role_permission) && in_array($permission->id,
