...
<script src="https://pomax.github.io/svg-path-reverse/reverse.js"></script>
<script>
  const dimensionGroups = [
    { name: "Arah Kerja", startIndex: 0, length: 3 },
    { name: "Kepemimpinan", startIndex: 3, length: 3 },
    { name: "Aktifitas", startIndex: 6, length: 2 },
    { name: "Pegaulan", startIndex: 8, length: 4 },
    { name: "Gaya Kerja", startIndex: 12, length: 3 },
    { name: "Sifat", startIndex: 15, length: 3 },
    { name: "Ketaatan", startIndex: 18, length: 2 },
  ];
 /* const papiData = [
    {
      name: "N",
    ...
    },
    {
      ...
      value: 7,
      desc: "Peran",
      desc2: "Penuh Semangat"
    },
    {
      name: "X",
      value: 9,
      desc: "Kebutuhan",
      desc2: "Untuk Diperhatikan"
    },
    {
      name: "S",
      value: 2,
      desc: "Peran",
      desc2: "Hubungan Sosial"
    },
    {
      name: "B",
      value: 1,
      desc: "Kebutuhan",
      desc2: "Untuk Diterima"
    },
    {
      name: "O",
      value: 6,
  ...
  const papiData = {!! json_encode($result->graph) !!};
  ...
  const dimensionSliceBg2 = "#071a52";
  ...
  const dimensionGroupArc = (startIndex, length) => {
    ...
    return d3
      .arc()
      .outerRadius(containerSize / 2 - 10)
  ...
  const dimensionsArc = (index) => {
    ...
    return d3
      .arc()
      .outerRadius(containerSize / 2 - 150)
      .innerRadius(containerSize / 2 - 125)
      .startAngle(degToRad(startDeg))
      .endAngle(degToRad(endDeg))
      .cornerRadius(sliceCornerRad);
  };
  ...
  // dimension Kurnia
  const dimensionsArc2 = (index) => {
    const startDeg = chartRotateAngle + index * (sliceAngle + padAngle);
    const endDeg = startDeg + sliceAngle;
    return d3
      .arc()
      .outerRadius(containerSize / 2 - 55)
      .innerRadius(containerSize / 2 - 120)
      .startAngle(degToRad(startDeg))
      .endAngle(degToRad(endDeg))
      .cornerRadius(sliceCornerRad);
  };
  ...
  papiData.forEach((d, i) => {
    dimensionSlices
      .append("path")
      .attr("class", "dimensionSlice2")
      .attr("id", `dimensionSlice2_${i}`)
      .attr("d", dimensionsArc2(i))
      .attr("fill", dimensionSliceBg2)
      .attr("stroke", "none");

    // Calculate the end angle in radians
    const endAngle = dimensionsArc2(i).endAngle()();

    //Create a new invisible arc that the text can flow along
    dimensionSlices
      .append("path")
      .attr("class", "dimensionSlice2_centeredTextContainer")
      .attr("id", `dimensionSlice2_centeredTextContainer_${i}`)
      .attr("d", invisibleArcHelper(`#dimensionSlice2_${i}`, endAngle))
      .style("fill", "none");

    // Apending Dimension Group Title to the slices
    dimensionSlices
      .append("text")
      .attr("class", "dimensionSlice2_centeredText_title")
      .attr("dy", () => {
        if (endAngle > (90 * Math.PI) / 180 && endAngle < (270 * Math.PI) / 180)
          return -8;
        else return 18;
      ...
      .style("text-anchor", "middle")
      .style("font-weight", 600)
      .attr("xlink:href", `#dimensionSlice2_centeredTextContainer_${i}`)
      .text(() => d.desc);
  ...
  const scaleArc = (index, scale) => {
    const outer = containerSize / 2 - 160 - ((9 - scale) * containerSize) / 33;
