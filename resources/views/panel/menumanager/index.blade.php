...
@section('content')
  <div class="row">
    ...
    <div class="col-sm-6 col-lg-6">
      <form id="changeHierarchy" class="formStore"
            action="{{ route('panel.menu-manager.changeHierarchy') }}">
        @method('POST')
        @csrf
        <div class="card">
          ...
          <div class="card-footer d-flex justify-content-end">
            ...
            <button type="submit" class="btn btn-sm btn-warning" style="display: none"><i
                class="fa-solid fa-floppy-disk"></i> Ubah
            </button>
          </div>
        </div>
      </form>
      ...
      <form id="formMenumanager" action="{{ $config['form']->action }}">
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div id="errorCreate" class="mb-3" style="display:none;">
              <div class="alert alert-danger" role="alert">
                <div class="alert-icon"><i
                    class="flaticon-danger text-danger"></i></div>
            ...
            <fieldset class="mb-3">
              ...
              <div class="form-check form-check-inline">
                <input type="radio" name="type" class="form-check-input" value="module"
                       id="exampleRadio1" {{ (!isset($data) ? 'checked' : ($data->type == 'module' ? 'checked' : '' ))  }}>
                <label class="form-check-label" for="exampleRadio1">Module</label>
              </div>
              <div class="mb-3 form-check form-check-inline">
                <input type="radio" name="type" class="form-check-input" value="header"
                       id="exampleRadio2" {{ (isset($data->type) && $data->type == 'header') ? 'checked' : '' }}>
                <label class="form-check-label" for="exampleRadio2">Header</label>
              </div>
              <div class="mb-3 form-check form-check-inline">
                <input type="radio" name="type" class="form-check-input" value="line"
                       id="exampleRadio3" {{ (isset($data->type) && $data->type == 'line') ? 'checked' : '' }}>
                <label class="form-check-label" for="exampleRadio3">Line</label>
              </div>
              <div class="mb-3 form-check form-check-inline">
                <input type="radio" name="type" class="form-check-input" value="static"
                       id="exampleRadio4" {{ (isset($data->type) && $data->type == 'static') ? 'checked' : '' }} >
                <label class="form-check-label" for="exampleRadio4">Static</label>
              </div>
            </fieldset>
            <div class="mb-3"
                 style="{{ (isset($data) && $data->type == 'line') ? 'display:none;' : '' }}">
              <label>Nama <span class="text-danger">*</span></label>
              <input type="text" name="title" class="form-control"
                     placeholder="ex: Menu Manager"
                     value="{{ (isset($data) ?? $data->title) ? $data->title : '' }}"/>
            </div>
            <div class="mb-3"
                 style="{{ (isset($data) && ($data->type == 'line' || $data->type == 'header' || $data->type == 'static' )) ? 'display:none;' : '' }}">
              <label>Slug</label>
              <input type="text" name="slug" class="form-control"
                     placeholder="ex: menu-manager"
                     value="{{ (isset($data) ?? $data->slug) ? $data->slug : '' }}"/>
            </div>
            <div class="mb-3"
                 style="{{ (isset($data) && ($data->type == 'line' || $data->type == 'header' || $data->type == 'static')) ? 'display:none;' : '' }}">
          ...
          <div class="card-footer justify-content-between border-top">
            <button type="submit" class="btn btn-primary float-end">Simpan <i
...
@push('style')
  <link rel="stylesheet" href="{{ asset('assets/libs/nestable/nestable.css') }}">
@endpush

@push('script')
  <script src="{{asset('assets/libs/nestable/nestable.js')}}"></script>
  <script>
